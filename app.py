...
async def protect_public_gallery(request: Request, call_next):
    ...
    if PUBLIC_GALLERY_ONLY:
        ...
        public_route = (request.method in {"GET", "HEAD"} and (
...
class GallerySettingsPayload(BaseModel):
    ...
    pinterest_url: str = ""
    instagram_url: str = ""
    contact_email: str = os.environ.get("BLACKCANVAS_CONTACT_EMAIL", "")
...
def get_gallery_settings() -> dict[str, str | bool]:
    ...
    try:
        ...
        return {**defaults, "public_mode": PUBLIC_GALLERY_ONLY}
    result = {**{key: str(saved.get(key, value)) for key, value in defaults.items()}, "public_mode": PUBLIC_GALLERY_ONLY}
    if PUBLIC_GALLERY_ONLY and os.environ.get("BLACKCANVAS_CONTACT_EMAIL"):
        result["contact_email"] = os.environ["BLACKCANVAS_CONTACT_EMAIL"].strip()
    return result
